use crate::{column::ColumnDescriptor, cpk_file::CpkFile, utf_table::UTFTable};

pub fn toc_finder(table: &UTFTable) -> Option<(u64, u64)> {
...
pub fn toc_reader(table: &UTFTable, content_offset: u64) -> Vec<CpkFile> {
    const INVALID: usize = usize::MAX;
        ...
        for (col_idx, column) in columns.iter().enumerate() {
            ...
            if let Some(data) = cell_data {
                if col_idx == dir_name_col_idx {
                    ...
                    cpk_file.directory = Some(read_utf_string(&string_pool, string_offset as usize));
